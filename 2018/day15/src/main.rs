...
impl EntityManager {
    fn set_elf_power(&mut self, elf_power: u8) {
        self.units
            .iter_mut()
            .flatten()
            .filter(|u| u.species == Species::Elf)
            .for_each(|elf| elf.ap = elf_power)
    }
...
fn solve() -> Result<(), Box<dyn Error>> {
    ...
    let world = input.parse::<World>().unwrap();

    let some = level1(world.clone());
